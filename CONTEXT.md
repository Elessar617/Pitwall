...
3. **Repeatable process?** Yes → use a skill from `.claude/skills/` (14 available). No → prompt directly.
...
| **Work on live timing / track-position rendering (F1 session)** | `build/CONTEXT.md` | `.claude/reference/frontend-stack.md` (Textual), `.claude/reference/project-architecture.md`, `.claude/reference/external-resources.md` (OpenF1) |
| **Work on the season tracker (schedule / standings / results)** | `build/CONTEXT.md` | `.claude/reference/project-architecture.md`, `.claude/reference/external-resources.md` (FastF1, Jolpica/Ergast) |
| **Work on the strategy mini-game (tyre + pit-plan, race scoring)** | `build/CONTEXT.md` | `.claude/reference/project-architecture.md`, `.claude/reference/glossary.md` (DRS, tyre compounds, parc fermé) |

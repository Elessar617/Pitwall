...
It's built **on top of the workspace-blueprint scaffold** — an agent-native template for development. The scaffold provides the workspaces (`spec/`, `lab/`, `build/`, `ship/`), the four-agent build loop (planner → implementer ↔ reviewer ↔ adversary), and the `.claude/` infrastructure. If you want to use the same scaffold for a different project, see the upstream [workspace-blueprint](https://github.com/Elessar617/workspace-blueprint) repo (`scripts/update-scaffold.sh`).
...
│  ├─ skills/            ← on-demand procedures (28 skills: 10 project + 10 integrations + 4 routing-vendored + 4 curated-vendored)
│  ├─ agents/            ← planner, implementer, reviewer, adversary specs
│  ├─ reference/         ← looked up on demand (F1 + Pitwall facts live here)
│  ├─ hooks/             ← 11 bash hooks + 5 routing hooks (incl. `route-inject.mjs`) enforcing rules by construction
...
│  (reference: the NHL TUI https://github.com/vgreg/faceoff — structural inspiration; local copy removed 2026-06-10)
│
...
- **28 skills** in `.claude/skills/` — 6 project-specific (tdd-loop, bug-investigation, refactor-protocol, spike-protocol, spec-authoring, data-analysis) + 12 process (anti-micromanage, architecture-audit, caveman, finishing-a-development-branch, handoff, how-to-prompt, requesting-code-review, scope-discipline, trust-the-gate, verification-before-completion, write-a-skill, zoom-out) + 4 office (docx, pptx, xlsx, pdf — source-available local bundles, untracked) + 4 routing-vendored (systematic-debugging, writing-plans, brainstorming, karpathy-guidelines — MIT-attributed in `.claude/skills/THIRD_PARTY_LICENSES.md`, refreshed via `npm run refresh-vendored`)
...
Bootstrap procedure for using this scaffold in another repo: see the upstream workspace-blueprint repo (`scripts/update-scaffold.sh` + `scripts/bootstrap.sh`).
